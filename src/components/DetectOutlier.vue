  ...
  methods: {
    ...
    async detectOutliers(useAgeCheck) {
      ...
      try {
        ...
        const response = await axios.post('https://back-csv-analyzer-production.up.railway.app/detect_outliers', formData, {
          headers: {
    ...
    async checkTaskResult(taskId) {
      try {
        const response = await axios.get(`https://back-csv-analyzer-production.up.railway.app/task_result_outlier/${taskId}`, {
          withCredentials: true, // Inclure les cookies
        });
        this.taskStatus = null;
