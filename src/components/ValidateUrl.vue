    ...
    <div class="row custom-bg-top justify-content-center align-items-center">
      <div class="col-md-6 text-center">
        ...
        <div class="title-container">
          ...
          <tr></tr>
          <h6 class="subtitle-text" style="color:black;">Validez en quelques clicks les url de vos fichiers</h6>
        </div>
    ...
    <div class="row custom-bg-second-top justify-content-center align-items-center">  -->
      <div class="col-md-6">
        <div class="file-upload card p-4">
          <h6>
            <a href="/validate-url" @click="reloadPage" style="margin-left: 520px; text-decoration: none;color:rgba(62, 77, 207, 0.884);" class="subtitle-text"><i class="fas fa-undo"></i> <!-- Icône de retour --></a>
          </h6>
          ...
          <div v-if="file" class="file-preview mt-4">
            <div class="card p-3">
              <h4 class="card-title">Fichier sélectionné:</h4>
              <ul class="list-unstyled">
                <li><i class="fas fa-file-alt"></i> {{ file.name }}</li>
              </ul>

              <!--<div class="url-column-container mt-3">
                <input v-model="urlColumn" type="text" placeholder="Nom de la colonne URL" class="url-column-input form-control" />
              </div>
               Dropdown list for selecting the column -->
              <div class="url-column-container mt-3">
                <select v-model="urlColumn" class="form-control">
          ...
          <div v-if="errorMessage" class="error-message alert alert-danger mt-3">
            <p>Les details de l'erreur sont disponibles en bas.</p>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    reloadPage(event) {
      ...
      window.location.href = '/validate-url'; // Redirige vers la nouvelle page
    ...
    async uploadFile() {
      ...
      if (!this.urlColumn) {
        this.errorMessage = 'Veuillez spécifier le nom de la colonne URL.';
        this.notification = null; // Hide notification
        this.isLoading = false; // Arrêter de charger
        return;
      }
      ...
      try {
        await axios.get('https://fb060ae8c88513c60a6c97a5989c736f.serveo.net/', {
        withCredentials: true
        });
        // Envoyer le fichier au backend
        const response = await axios.post('https://fb060ae8c88513c60a6c97a5989c736f.serveo.net/validate_urls', formData, {
          headers: {
        ...
        const interval = setInterval(async () => {
          try {
            const statusResponse = await axios.get(`https://fb060ae8c88513c60a6c97a5989c736f.serveo.net/task_status/${taskId}`, {
              withCredentials: true, // Inclure les cookies
            });
            ...
            if (status.state === 'SUCCESS') {
              ...
              this.downloadUrl = `https://fb060ae8c88513c60a6c97a5989c736f.serveo.net/download_clean_csv/${taskId}`;
              this.taskStatus = 'Validation des URLs terminée avec succès.';
              this.notification = null; // Hide notification
              this.isLoading = false; // Arrêter de charger
            } else if (status.state === 'FAILURE') {
              clearInterval(interval);
              this.taskStatus = 'Erreur lors de la validation des URLs.';
